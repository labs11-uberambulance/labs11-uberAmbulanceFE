...
class OnBoardingForm extends Component {
    ...
    submitForm = async () => {
      ...
      const storageRef = firebase.storage().ref(`profile_images/${this.props.id}@${new Date().toISOString()}`);
      const uploadTask = storageRef.put(image, { contentType: image.type });
      return uploadTask.on('state_changed', (snapshot) => {//if we wont handle progress change .on() to .then()
    ...
    return (
      <div>
        ...
        <div className="inputHolder">
            ...
            />
            <Button type="button" onClick={() => this.scrollToNextInputHandler(this.rateForScroll)}>Next</Button>
        </div>
        <Button 
              type="button" 
              color="primary" 
              onClick={() => this.photoInp.current.click()}
            >{this.state.file ? `${this.state.file.name}` : 'Set Profile Image *'}</Button>
            <input 
              style={{display:'none'}} 
              accept="image/*" 
              type="file" 
        ...
        <div className="inputHolder" style={{paddingTop: '0'}}>
            <TextField
              ...
              helperText={`We recommend $2${this.state.rateInp !== '' ? `, you pledge to never charge more than $${this.state.rateInp} per 10km.` : '.' }`}
            />
